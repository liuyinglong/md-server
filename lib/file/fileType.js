...
let fsPaht = require("path");

function TypeFile(path, fileType) {
    this.path = path;
    this.fileType = fileType;
    this.fileList = this.getFiles(path, fileType);
    this.event = emitter;
...
TypeFile.prototype = {
    ...
    getFiles: function (path, fileType) {
        ...
        itemList.forEach(function (item) {
            let tempPath =fsPaht.join(path,item);
    ...
    watch: function (path, fileType) {
        ...
        fs.watch(path, function (e, filename) {
            ...
            let tempPath = fs.lstatSync(path + "/" + filename);
            if (tempPath.isDirectory()) {
                changeFileList = self.getFiles(tempPath, fileType);
            }
            if (tempPath.isFile()) {
                changeFileList.push(tempPath);
            }
        });
        // self.event.emit("file_change", changeFileList);
    }
};
